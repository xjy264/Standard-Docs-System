...
- `PUT /api/doc-nodes/{id}`：本科室用户或超级管理员修改文件夹或文件名称、排序；文件夹节点可同步修改年份和是否显示上传进度，未传时默认不显示进度；文件节点可同步修改文件类型、富文本内容、业务类型、提交模式、上传截止时间、车间上传选项、可见车间范围和收集项名称及说明，前端文件编辑弹窗不展示年份选择。
